...
const SignIn = () => {
    ...
    const [signInSuccess, setSignInSuccess] = useState<null | boolean>(null);
    ...
    return(
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button type="submit" className="w-full cursor-pointer font-semibold rounded-full bg-blue-300 py-2 text-base text-slate-900 hover:bg-opacity-70">
            {loading ? 'Signing in...' : 'Sign in'}
        </button>
        {signInSuccess === false && <p className="text-red-500">Sign in failed.</p>}
        </form>
    )
}
